...
class CustomerController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->model()->where('is_customer', User::CUSTOMER)->orderBy('id', 'desc');
        ...
        $grid->filter(function($filter) use($saleStaff) {
            ...
            $filter->column(1/3, function ($filter) use ($saleStaff) {
                ...
                }, 'Số dư', 'wallet')->select(['Công nợ', 'Số dư', 'Tất cả'])->default(2);
            ...
            $filter->column(1/3, function ($filter) {
                ...
                $filter->equal('province', 'Tỉnh / TP')->select(Province::all()->pluck('name', 'province_id'));
            });
            $filter->column(1/3, function ($filter) use($saleStaff) {
                $filter->equal('staff_sale_id', 'Sale')->select($saleStaff);
                $filter->where(function ($query) {
                    if ($this->input == '0') {
                ...
                }, 'Sắp xếp tiền', 'wallet_sort')->select(['Giảm dần', 'Tăng dần']);
                $filter->where(function ($query) {
                    if ($this->input == '0') {
        ...
        $grid->header(function ($query) {
            ...
            return '<h4>CÔNG NỢ : <span style="color:'.$color.'">&nbsp;'. number_format($owed) ."</span> (VND)</h4>";
        });
        ...
        $grid->email("Thông tin")->display(function () {
            $warehouse = $this->warehouse->name ?? "";
            $html = "<ul style='padding-left: 10px'>";
            $html .= "<li>Email: ".$this->email."</li>";
            $html .= "<li>Họ và tên: ".$this->name."</li>";
            $html .= "<li>SDT: ".$this->phone_number."</li>";
            $html .= "</ul>";
        ...
        $grid->address('Địa chỉ')->display(function () {
            $province = Province::whereProvinceId($this->province)->first()->name ?? "";
            $district = District::whereDistrictId($this->district)->first()->name ?? "";
        ...
        if (Admin::user()->isRole('head_sale'))
        {
            ...
            $grid->staff_sale_id('Nhân viên kinh doanh')->display(function () {
                return User::find($this->staff_sale_id)->name ?? "";
            });
        }
        $grid->note('Ghi chú')->editable()->width(150);
        $grid->paginate(3);
        // $grid->disableActions();
        $grid->actions(function ($actions) {
            ...
            $actions->prepend(view('admin.grid-actions.customer-recharge')->render());
        });
        $grid->disableColumnSelector();
        // $grid->batchActions(function ($batch) {
        //     $batch->add(new Recharge());
        // });
        return $grid;
    }
    ...
    public function rechargeForm($id) {
        ...
        $form->tools(function (Form\Tools $tools) {
            ...
            $tools->disableView();
            $tools->disableList();
            $tools->append('<a href="'.route('admin.customers.index').'" class="btn btn-sm btn-default" title="Danh sách">
